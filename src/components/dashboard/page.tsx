  ...
  return (
    <div className="p-6 space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <StatCard 
          title="Totaal Vechters" 
          value={stats.counts.fighters} 
          icon={Users} 
          description="Geregistreerde atleten"
        />
        <StatCard 
          title="Sportscholen" 
          value={stats.counts.gyms} 
          icon={Dumbbell} 
          description="Actieve locaties"
        />
        <StatCard 
          title="Evenementen" 
          value={stats.counts.events} 
          icon={Calendar} 
          description="Gepland & Afgerond"
        />
        <StatCard 
          title="Nieuwsberichten" 
          value={stats.counts.news} 
          icon={Newspaper} 
          description="Laatste updates"
        />
      </div>

      {/* Snelkoppelingen / Highlights */}
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/dashboard/events">
              <Button variant="link" className="text-red-400 p-0 h-auto">Bekijk alles <ArrowRight className="ml-1 h-4 w-4"/></Button>
            </Link>
          </div>
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/dashboard/news">
              <Button variant="link" className="text-red-400 p-0 h-auto">Lees meer <ArrowRight className="ml-1 h-4 w-4"/></Button>
            </Link>
          </div>
          {stats.featured.latestNews ? (
            <div>
              ...
              <p className="text-slate-600 text-xs mt-2">
                Gepubliceerd op {format(new Date(stats.featured.latestNews.publishedAt), 'dd/MM/yyyy')}
              </p>
            </div>
          ) : (
